...
var rules []*glr.Rule
var states []*glr.ParseState

func init() {
	var err error
	rules, states, err = glr.LoadGrammarRulesAndStates("datetime_yacc.y", "datetime_yacc.states.txt")
	if err != nil {
		panic(fmt.Sprintf("Failed to load rules and states: %v", err))
	}
}
...
func ExtractDatetimeRanges(mode, in string) (*DatetimeRanges, error) {
	defer func() {
	...
	forest, err := glr.Parse(rules, states, NewDatetimeLexer(in))
	if yyDebug == 3 {
	...
	dtrs := forest[0].Children[0]
	rs := &DatetimeRanges{}
	for _, dtr := range dtrs.Children {
